...
exports.getReceiptPDF = exports.previewReceipt = exports.getMassageServices = exports.resendReceipt = exports.getReceiptById = exports.getReceipts = exports.createReceipt = exports.createAndSendReceipt = void 0;
const zod_1 = require("zod");
const database_1 = __importDefault(require("../../config/database"));
const errorHandler_1 = require("../../middleware/errorHandler");
const pdfkit_1 = __importDefault(require("pdfkit"));
const nodemailer_1 = __importDefault(require("nodemailer"));
const massages_1 = require("../../config/massages");
...
const createReceiptSchema = zod_1.z.object({
    ...
    noteId: zod_1.z.string().optional(), // Optionnel - lien vers une note si existante
    serviceId: zod_1.z.string().optional(),
    serviceName: zod_1.z.string().min(1, 'Le nom du service est requis'),
    duration: zod_1.z.number().int().min(1, 'La durée est requise (50, 60 ou 80 minutes)'),
    treatmentDate: zod_1.z.string().min(1, 'La date du traitement est requise'),
    treatmentTime: zod_1.z.string().min(1, 'L\'heure du traitement est requise'),
});
/**
 * Génère un numéro de reçu unique par thérapeute
 * Chaque thérapeute a son propre compteur qui commence à 1
 */
const generateReceiptNumber = async (therapistId) => {
    // Trouver le dernier reçu du thérapeute
    const lastReceipt = await database_1.default.receipt.findFirst({
        where: {
            therapistId,
        },
        orderBy: {
    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            doc
                ...
                .font('Helvetica')
                .fillColor('#666')
                .text('Ce reçu est valide pour les réclamations d\'assurance. Veuillez le conserver pour vos dossiers.', leftMargin, currentY + 8, { align: 'center', width: pageWidth - 80 })
                .fontSize(6)
                .fillColor('#999')
                .text(`Document généré le ${new Date().toLocaleDateString('fr-CA')}`, leftMargin, currentY + 22, { align: 'center', width: pageWidth - 80 });
            doc.end();
        }
        catch (error) {
            reject(error);
 ...
 */
const sendReceiptEmail = async (clientEmail, clientName, receiptNumber, pdfBuffer, receiptDetails) => {
 ...
 */
const createAndSendReceipt = async (req, res) => {
    const user = req.user;
    // Vérifier que l'utilisateur est un massothérapeute, esthéticienne ou admin
    if (user.role !== 'MASSOTHERAPEUTE' &&
        user.role !== 'ESTHETICIENNE' &&
        user.role !== 'ADMIN') {
        throw new errorHandler_1.AppError('Accès refusé. Seuls les thérapeutes peuvent créer des reçus.', 403);
    }
    ...
    if (!therapist) {
        throw new errorHandler_1.AppError('Thérapeute non trouvé', 404);
    }
    // Vérifier que le thérapeute a un numéro d'ordre
    if (!therapist.numeroOrdre) {
        throw new errorHandler_1.AppError('Vous devez avoir un numéro d\'ordre professionnel pour émettre des reçus. Veuillez contacter l\'administrateur.', 400);
    }
    // Si une note est fournie, vérifier qu'un reçu n'existe pas déjà pour cette note
    if (validatedData.noteId) {
        const existingReceipt = await database_1.default.receipt.findUnique({
            where: { noteId: validatedData.noteId },
        });
        if (existingReceipt) {
            throw new errorHandler_1.AppError('Un reçu existe déjà pour cette note', 400);
        }
    }
    ...
    const subtotal = (0, massages_1.getMassagePrice)(validatedData.serviceName, validatedData.duration);
    if (subtotal === null) {
        throw new errorHandler_1.AppError(`Prix non trouvé pour le massage "${validatedData.serviceName}" avec une durée de ${validatedData.duration} minutes.`, 400);
    }
    const taxes = (0, massages_1.calculateTaxes)(subtotal);
    ...
    const receipt = await database_1.default.receipt.create({
        data: receiptData,
    });
    ...
    try {
        await sendReceiptEmail(client.courriel, `${client.prenom} ${client.nom}`, receiptNumber, pdfBuffer, {
        ...
        await database_1.default.receipt.update({
            where: { id: receipt.id },
            data: {
                emailSent: true,
        ...
        console.error('Erreur lors de l\'envoi de l\'email:', error);
        throw new errorHandler_1.AppError('Le reçu a été créé mais l\'email n\'a pas pu être envoyé. Veuillez réessayer.', 500);
    }
};
exports.createAndSendReceipt = createAndSendReceipt;
/**
...
const createReceipt = async (req, res) => {
    ...
    const subtotal = (0, massages_1.getMassagePrice)(validatedData.serviceName, validatedData.duration);
    if (subtotal === null) {
        throw new errorHandler_1.AppError(`Prix non trouvé pour le massage "${validatedData.serviceName}" avec une durée de ${validatedData.duration} minutes. Veuillez vérifier le nom du service et la durée.`, 400);
    }
...
const getReceiptById = async (req, res) => {
    ...
    if (!receipt) {
        throw new errorHandler_1.AppError('Reçu non trouvé', 404);
    }
    // Vérifier les permissions (non-ADMIN ne peuvent voir que leurs propres reçus)
    if (user.role !== 'ADMIN' && receipt.therapistId !== user.id) {
        throw new errorHandler_1.AppError('Vous n\'avez pas accès à ce reçu', 403);
    }
...
const resendReceipt = async (req, res) => {
    ...
    // Renvoyer l'email
    try {
        await sendReceiptEmail(receipt.clientEmail, receipt.clientName, receipt.receiptNumber, pdfBuffer, {
            serviceName: receipt.serviceName,
            duration: receipt.duration,
            total: receipt.total,
            treatmentDate: receipt.treatmentDate,
        ...
        throw new errorHandler_1.AppError('Impossible de renvoyer le reçu. Veuillez réessayer.', 500);
    }
};
exports.resendReceipt = resendReceipt;
/**
 * @desc    Récupérer la liste des services de massage avec leurs prix
 * @route   GET /api/receipts/massage-services
 * @access  Privé (MASSOTHERAPEUTE, ESTHETICIENNE, ADMIN)
 */
const getMassageServices = async (req, res) => {
    res.status(200).json({
        success: true,
        data: massages_1.MASSAGE_SERVICES,
    });
};
exports.getMassageServices = getMassageServices;
/**
...
exports.previewReceipt = previewReceipt;
/**
